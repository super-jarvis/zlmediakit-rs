...
/// Read until we have a complete RTSP response (headers + body).
...
/// Send an RTSP request and read the full response. Returns (status_code, response_string).
...
/// Drain interleaved data from buffer.
...
/// Extract a header value from a full RTSP response string.
...
async fn rtsp_e2e_publish_receives_media() {
    ...
    let announce = format!(
        "ANNOUNCE rtsp://127.0.0.1:{}/live/test RTSP/1.0\r\n\
         CSeq: 2\r\n\
    ...
    let setup = format!(
        "SETUP rtsp://127.0.0.1:{}/live/test/track1 RTSP/1.0\r\n\
         CSeq: 3\r\n\
    ...
    // RECORD
    let record = format!(
        "RECORD rtsp://127.0.0.1:{}/live/test RTSP/1.0\r\n\
         CSeq: 4\r\n\
         Session: {}\r\n\r\n",
    ...
    // Send SPS NALU (type 7) via RTP, marker bit set
    let nalu_sps = make_rtp_packet(&sps, pt, 0, ts, ssrc, true);
    stream
    ...
    // Send PPS NALU (type 8) via RTP, marker bit set
    let nalu_pps = make_rtp_packet(&pps, pt, 1, ts, ssrc, true);
    stream
    ...
    // Send IDR NALU (type 5, key frame) via RTP, marker bit set
    let idr = [0x65u8, 0x9a, 0x00, 0x15, 0x20]; // NAL header 0x65 = type 5
    let nalu_idr = make_rtp_packet(&idr, pt, 2, ts, ssrc, true);
    stream
    ...
    let source = mgr.get("__defaultVhost__", "live", "test");
    assert!(source.is_some(), "source should exist after RTSP publish");
    ...
    // TEARDOWN
    let teardown = format!(
        "TEARDOWN rtsp://127.0.0.1:{}/live/test RTSP/1.0\r\n\
         CSeq: 5\r\n\
         Session: {}\r\n\r\n",
        TEST_PORT, session_id
    );
